...
const SECTION_TYPES: SectionType[] = ["hero", "about", "products", "usp", "testimonials", "contact", "custom"];
...
function SectionsContent({ storeId }: { storeId: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setLoadError(err instanceof ApiError ? err.message : "Could not load sections.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="px-6 py-10 sm:px-10">
      <div className="max-w-3xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Website Sections</h1>
          <p className="text-slate-500 mt-2">Manage the content blocks that make up your storefront's homepage.</p>
        </div>
        ...
        <section className="bg-white border border-slate-200 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-4">Homepage Layout</h2>
          {loading ? (
            ...
            <p className="text-slate-500 text-sm">No sections yet. Add your first one above.</p>
          ) : (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-900 truncate">{section.title}</p>
