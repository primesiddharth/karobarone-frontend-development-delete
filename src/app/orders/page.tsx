...
import { orders, money } from "@/components/commerce/data";
          ...
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  {["Order", "Date", "Items", "Amount", "Status", "Action"].map(
                    (h) => (
                      <th
                        key={h}
                        className="border-b border-gray-200 px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500"
                      >
                        {h}
                      </th>
                    ),
                ...
                {filtered.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50/70">
                    ...
                    <td className="px-5 py-4">
                      <Link
                        href="#"
                        className="inline-flex items-center gap-1 text-sm font-medium text-[#5b4ef9]"
                      >
            ...
            {filtered.map((o) => (
              <div key={o.id} className="p-5">
                <div className="flex justify-between gap-3">
                  ...
                  <span
                    className={`h-fit rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(o.status)}`}
                  >
                    {o.status}
                  </span>
                </div>
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <Link href="#" className="text-sm font-medium text-[#5b4ef9]">
                    View order
                  </Link>
                </div>
              </div>
